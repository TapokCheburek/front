          ...
          <div className="w-full md:w-2/3 flex flex-col">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                <span className="text-2xl md:text-3xl font-normal">{product.rating.toString().replace('.', ',')}</span>
            ...
            <div className="grid grid-cols-2 gap-y-4 text-xl mb-auto">
              ...
              <div className="text-black">{product.price} р {product.priceOpt ? `| ${product.priceOpt} р (опт)` : ''}</div>
              ...
              <div className="text-black">{product.baseType}</div>
              
              <div className="text-gray-600">Мощность</div>
              <div className="text-black">{product.power}</div>
              
              <div className="text-gray-600">Температура света</div>
              <div className="text-black">{product.colorTemp}</div>
            </div>
            ...
            <div className="flex items-center gap-6 mt-8 flex-wrap">
              ...
              <button 
                ...
                className="bg-blue-500 hover:bg-blue-600 text-black px-12 py-3 rounded-xl text-xl flex-1 max-w-[280px] transition-colors"
               >
                В корзину
              </button>
            </div>
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-col gap-4">
          {product.reviews.map((review, idx) => (
            <div key={idx} className="bg-[#e5e7eb] rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg text-black">{review.author}</h3>
              </div>
              <div className="flex items-center justify-between mb-4">
                 <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    <Star 
                      key={i} 
                      className={`w-6 h-6 ${i < review.rating ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-300 text-gray-300'}`} 
                    />
                  ))}
                </div>
          ...
          {product.reviews.length === 0 && (
            <div className="text-gray-500 p-4">Пока нет отзывов</div>
          )}
        </div>
      </div>
    </div>
